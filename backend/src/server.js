...
const asyncRoute=fn=>(req,res,next)=>Promise.resolve(fn(req,res,next)).catch(next); const isValidObjectId=value=>mongoose.isValidObjectId(value); const current=async()=>Hackathon.findOne({status:{$nin:['closed','archived']}}).sort({createdAt:-1});
...
app.post('/api/attendance/scan',auth,allow('admin','judge','mentor','volunteer'),asyncRoute(async(req,res)=>{
  const requestedEventId=req.body?.hackathonId||req.body?.eventId; const h=(isValidObjectId(requestedEventId)?await Hackathon.findById(requestedEventId):null)||await current(); if(!h)return res.status(404).json({message:'Hackathon not found'});
  if(h.settings?.features?.attendance===false)return res.status(503).json({message:'Attendance is currently disabled for this event.'});
...
app.delete('/api/admin/certificates/:id',auth,allow('admin'),asyncRoute(async(req,res)=>{if(!isValidObjectId(req.params.id))return res.status(400).json({message:'Invalid certificate id.'}); const c=await Certificate.findByIdAndDelete(req.params.id);if(!c)return res.status(404).json({message:'Certificate not found'});res.json({message:'Certificate deleted'})}));
app.patch('/api/admin/certificates/:id',auth,allow('admin'),asyncRoute(async(req,res)=>{if(!isValidObjectId(req.params.id))return res.status(400).json({message:'Invalid certificate id.'}); const c=await Certificate.findById(req.params.id);if(!c)return res.status(404).json({message:'Certificate not found'});if(req.body.type!=null)c.type=String(req.body.type).trim();if(req.body.fileUrl!==undefined)c.fileUrl=req.body.fileUrl||undefined;if(req.body.template&&typeof req.body.template==='object')c.template={...(c.template?.toObject?.()||c.template||{}),...req.body.template};await c.save();await audit(AuditLog,req.user,'UPDATE','Certificate',c._id,{certificateId:c.certificateId,type:c.type,hackathon:c.hackathon});res.json(await c.populate([{path:'user',select:'name email enrollmentId college'},{path:'hackathon',select:'name college venue logoUrl'}]))}));
...
app.patch('/api/notifications/:id/read',auth,asyncRoute(async(req,res)=>{
  if(!isValidObjectId(req.params.id))return res.status(400).json({message:'Invalid notification id.'}); const n=await Notification.findOneAndUpdate({_id:req.params.id,user:req.user._id},{read:true},{returnDocument:'after'});
...
async function connectMongoWithRetry(uri){
  ...
  for(let attempt=1;attempt<=attempts;attempt++){
    try{
      ...
      mongoose.connection.on('disconnected',()=>console.error('MongoDB disconnected. Mongoose will attempt to reconnect.'));
